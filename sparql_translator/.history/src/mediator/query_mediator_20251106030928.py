...
# When this module is executed directly (python src/mediator/query_mediator.py),
# Python's import path will not include the repository root, so absolute imports
# like `src.parser...` fail with ModuleNotFoundError. Try the normal imports
# first; if they fail, add the project root (two levels up from `src`) to
# sys.path and retry.
try:
    ...
    # file layout: <project_root>/src/mediator/query_mediator.py
