...
public class OWLAxiomsRendererVisitor extends IndentedRendererVisitor implements AlignmentVisitor, EDOALVisitor {
	...
	if ( align instanceof ObjectAlignment ) {
	    ...
	    alignment = align;
	    edoal = true;
            System.out.println("EDOALAlignment");
            try{
                ((EDOALAlignment) align).transformAlignmentToBeOWLable();
            }
            catch(AlignmentException aex){
                System.err.println(aex.getStackTrace());
            }
            System.out.println("EDOALAlignment");
	} else {
	    try {
        ...
        writer.print("<?xml version=\"1.0\"?>"+NL);
	...
	writer.print("</rdf:RDF>"+NL);
        System.out.println("End of OWL Rendering");
    }
	...
	indentedOutput("<owl:someValuesFrom>"+NL);
	/*The line above was added as there is no edoal way to signify that c is not universal*/
	increaseIndent();
	...
	indentedOutput("<owl:"+cardinality+" rdf:datatype=\"http://www.w3.org/2001/XMLSchema#nonNegativeInteger\">");
	writer.print(cardValue);
	...
	if ( owlop == null ) {
            System.err.println("Property problem : constructor : " + op);
            owlop = "TOSUPPRESS";
            //throw new AlignmentException( "Cannot translate property construction in OWL : "+op );
        }
	indentedOutput("<owl:"+owlop);
	...
	*/
	...
	if ( owlop == null ) {
            System.err.println("Property problem : constructor : " + op);
            //throw new AlignmentException( "Cannot translate relation construction in OWL : "+op );
            owlop = "TOSUPPRESS";
        }
	indentedOutput("<owl:"+owlop);
